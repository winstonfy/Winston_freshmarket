...
class OrderPayView(View):
    ...
    def post(self, request):
        ...
        # 初始化
        app_path=os.path.join(settings.BASE_DIR, 'apps/order/app_private_key.pem')
        alipay_path = os.path.join(settings.BASE_DIR, 'apps/order/alipay_public_key.pem')
        app_private_key_string = open(app_path).read()
        alipay_public_key_string = open(alipay_path).read()
        alipay = AliPay(
            appid="2016101800716862", # 应用id
